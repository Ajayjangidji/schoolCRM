...
type TabType = 'all' | 'teachers' | 'staff' | 'attendance';
...
export default function AdminStaffPage() {
  ...
  const perPage = 10;
  const [attendanceMap, setAttendanceMap] = useState<Record<string, 'present' | 'absent' | 'half-day' | 'leave'>>({});
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.tabRow}>
        ...
        <button className={`${styles.tab} ${activeTab === 'attendance' ? styles.tabActive : ''}`} onClick={() => { setActiveTab('attendance'); setCurrentPage(1); }}>
          <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" style={{ verticalAlign: '-2px', marginRight: '4px' }}>
            <rect x="3" y="3" width="14" height="14" rx="1" /><path d="M3 7h14M7 3v4M13 3v4" /><path d="M7 11l2 2 4-4" />
          </svg>
          Attendance
        </button>
      </div>
        ...
        <div className={styles.attendancePanel}>
          <div className={styles.attendanceHeader}>
            <h2 className={styles.attendanceTitle}>
              <svg width="18" height="18" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"><rect x="3" y="3" width="14" height="14" rx="1" /><path d="M3 7h14M7 3v4M13 3v4" /></svg>
              Today&apos;s Attendance — {new Date().toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
            </h2>
            <div className={styles.attendanceSummary}>
              <span className={styles.attStat} style={{ color: '#2e7d32' }}>Present: {Object.values(attendanceMap).filter(v => v === 'present').length}</span>
              <span className={styles.attStat} style={{ color: '#f59e0b' }}>Half Day: {Object.values(attendanceMap).filter(v => v === 'half-day').length}</span>
              <span className={styles.attStat} style={{ color: '#c62828' }}>Absent: {Object.values(attendanceMap).filter(v => v === 'absent').length}</span>
              <span className={styles.attStat} style={{ color: '#1565c0' }}>Leave: {Object.values(attendanceMap).filter(v => v === 'leave').length}</span>
              <span className={styles.attStat}>Unmarked: {allStaff.filter(p => p.status !== 'resigned').length - Object.keys(attendanceMap).length}</span>
            </div>
          </div>
          <div className={styles.attendanceList}>
            {allStaff.filter(p => p.status !== 'resigned').map((person, idx) => {
              const bgColor = AVATAR_COLORS[idx % AVATAR_COLORS.length];
              const status = attendanceMap[person.id];
              return (
                <div key={person.id} className={`${styles.attendanceRow} ${status ? styles[`att_${status.replace('-', '')}`] || '' : ''}`}>
                  <div className={styles.attPerson}>
                    <div className={styles.attAvatar} style={{ background: bgColor + '18', color: bgColor }}>{getInitials(person.name)}</div>
                    <div>
                      <div className={styles.attName}>{person.name}</div>
                      <div className={styles.attSub}>{person.employeeId} · {person.department}</div>
                    </div>
                  </div>
                  <div className={styles.attActions}>
                    {(['present', 'half-day', 'absent', 'leave'] as const).map((s) => (
                      <button
                        key={s}
                        className={`${styles.attBtn} ${status === s ? styles[`attBtn_${s.replace('-', '')}`] || styles.attBtnActive : ''}`}
                        onClick={() => setAttendanceMap(prev => {
                          if (prev[person.id] === s) { const next = { ...prev }; delete next[person.id]; return next; }
                          return { ...prev, [person.id]: s };
                        })}
                      >
                        {s === 'present' ? 'P' : s === 'half-day' ? 'HD' : s === 'absent' ? 'A' : 'L'}
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className={styles.attendanceFooter}>
            <button className={styles.submitBtn} onClick={() => alert(`Attendance saved for ${Object.keys(attendanceMap).length} employees!`)}>
              <svg width="14" height="14" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><path d="M5 10l3 3 7-7" /></svg>
              Save Attendance
            </button>
          </div>
        </div>
      ...
      {/* ── Filters ── */}
      {activeTab !== 'attendance' && <><div className={styles.filtersBar}>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.formGrid}>
                ...
                <div className={`${styles.formGroup} ${styles.formGroupFull}`}>
                  <label className={styles.formLabel}>Employee ID Proof (Aadhaar / PAN Card / Voter ID)</label>
                  <div className={styles.fileUploadZone}>
                    <input
                      type="file"
                      accept=".jpg,.jpeg,.png,.pdf"
                      onChange={(e) => setStaffIdFile(e.target.files?.[0] || null)}
                    />
                    {staffIdFile ? (
                      <div className={styles.fileChip}>
                        {staffIdFile.name}
                        <button className={styles.fileChipRemove} onClick={(e) => { e.stopPropagation(); setStaffIdFile(null); }}>×</button>
                      </div>
                    ) : (
            ...
            <div className={styles.modalFooter}>
              <button className={styles.cancelBtn} onClick={() => { setShowAddModal(false); setStaffIdFile(null); }}>Cancel</button>
              <button className={styles.submitBtn} onClick={() => { setShowAddModal(false); setStaffIdFile(null); alert('Employee added successfully!'); }}>Add Employee</button>
            </div>
          </div>
        </div>
      )}
